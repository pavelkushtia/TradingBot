...
from ..core.models import Portfolio, Order, Trade, Position, PerformanceMetrics
    ...
    async def save_performance_metrics(self, metrics: PerformanceMetrics) -> None:
        ...
        try:
            ...
            """, (
                ...
                float(metrics.average_win),
                float(metrics.average_loss),
                float(metrics.largest_win),
                float(metrics.largest_loss),
    ...
    async def get_performance_metrics(self) -> Optional[PerformanceMetrics]:
        ...
        try:
            ...
            cursor = await self.connection.execute("""
                ...
                FROM performance_metrics WHERE id = 1
            """)
            ...
            return PerformanceMetrics(
                ...
                profit_factor=Decimal(str(row[4])),
                total_trades=row[5],
                winning_trades=row[6],
                losing_trades=row[7],
                average_win=Decimal(str(row[8])),
                average_loss=Decimal(str(row[9])),
                largest_win=Decimal(str(row[10])),
                largest_loss=Decimal(str(row[11])),
                period_start=datetime.fromisoformat(row[12]),
                period_end=datetime.fromisoformat(row[13])
            )
