...
from ..core.models import Order, Trade, OrderStatus, OrderSide, OrderType
...
class ExecutionManager:
    ...
    async def shutdown(self) -> None:
        ...
        for order_id in list(self.pending_orders.keys()):
            try:
                ...
                self.logger.log_error(e, {"context": "shutdown_cancel", "order_id": order_id})
    ...
    async def submit_order(self, order: Order) -> Optional[Order]:
        ...
        try:
            ...
            self.logger.log_error(e, {"context": "order_submission", "order": order.dict()})
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            self.logger.log_error(e, {"context": "order_cancellation", "order_id": order_id})
            return False
    ...
    async def _validate_order(self, order: Order) -> bool:
        ...
        if order.type == OrderType.STOP and (not order.stop_price or order.stop_price <= 0):
    ...
    async def _submit_to_exchange(self, order: Order) -> None:
        ...
        #             order.id = result["id"]
        #         else:
        #             raise OrderExecutionError(f"Failed to submit order: {response.status}")
    ...
    async def _cancel_on_exchange(self, order: Order) -> None:
        ...
        #             raise OrderExecutionError(f"Failed to cancel order: {response.status}")
    ...
    async def _mock_fill_order(self, order: Order) -> None:
        ...
        try:
            ...
            fill_price = order.price if order.price else self._get_mock_market_price(order.symbol)
