...
class MarketDataManager:
    ...
    async def subscribe_symbols(self, symbols: List[str]) -> None:
        ...
        if new_symbols:
            self.logger.logger.info(f"Subscribing to {len(new_symbols)} new symbols: {list(new_symbols)}")
        ...
        try:
            ...
            async with self.session.get(url, params=params, headers=headers) as response:
                if response.status == 200:
                    ...
                    raise MarketDataError(f"Failed to fetch historical data: {error_text}")
                    
        except Exception as e:
            self.logger.log_error(e, {"context": "get_historical_bars", "symbol": symbol})
    ...
    async def _process_message(self, message: str) -> None:
        ...
        try:
            ...
            self.logger.log_error(e, {"context": "message_processing", "message": message[:100]})
    ...
    async def _handle_reconnection(self, error: Exception) -> None:
        ...
        if self.reconnect_attempts < self.max_reconnect_attempts:
            ...
            self.logger.logger.warning(
                f"Reconnecting to WebSocket (attempt {self.reconnect_attempts}/{self.max_reconnect_attempts}) in {delay}s..."
            )
            ...
            self.logger.logger.error(f"Max reconnection attempts reached. Market data disconnected.")
            raise MarketDataError(f"Failed to reconnect after {self.max_reconnect_attempts} attempts")
