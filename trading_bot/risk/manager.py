...
from ..core.models import StrategySignal, Portfolio, Position, Order
...
class RiskManager:
    ...
    async def evaluate_signal(self, signal: StrategySignal, portfolio: Optional[Portfolio]) -> bool:
        ...
        try:
            ...
            if not await self._check_daily_loss_limit(portfolio):
                self.logger.log_risk_event("daily_loss_limit_exceeded", {
            ...
            if not await self._check_max_positions(portfolio):
                self.logger.log_risk_event("max_positions_exceeded", {
                    "current_positions": len(portfolio.positions) if portfolio else 0,
            ...
            if not await self._check_position_size_limit(signal, portfolio):
                self.logger.log_risk_event("position_size_limit_exceeded", {
            ...
            if not await self._check_symbol_concentration(signal, portfolio):
                self.logger.log_risk_event("symbol_concentration_exceeded", {
                    "symbol": signal.symbol
                })
                return False
    ...
    async def calculate_position_size(
        ...
        try:
            ...
            if existing_position:
                ...
                max_additional = (portfolio.total_value * Decimal("0.1")) - current_exposure
                if max_additional <= 0:
                    return Decimal("0")
    ...
    async def calculate_stop_loss_price(self, position: Position, current_price: Decimal) -> Decimal:
    ...
    async def calculate_take_profit_price(self, position: Position, current_price: Decimal) -> Decimal:
    ...
    async def check_portfolio_risk(self, portfolio: Portfolio) -> Dict[str, Any]:
        ...
        try:
            ...
            concentration = float(max_position_value / total_value) if total_value > 0 else 0
            
            # Daily P&L as percentage
            daily_pnl_pct = float(portfolio.day_pnl / total_value) if total_value > 0 else 0
    ...
    async def _check_position_size_limit(self, signal: StrategySignal, portfolio: Optional[Portfolio]) -> bool:
        """Check if position size would exceed limits."""
        if not portfolio or not signal.price:
            return True
        
        # Calculate what the position size would be
        position_size = await self.calculate_position_size(signal.symbol, signal.price, portfolio)
        position_value = position_size * signal.price
        
        # Check against portfolio percentage limit
        max_position_value = portfolio.total_value * Decimal(str(self.max_position_size))
        
        return position_value <= max_position_value
    
    async def _check_symbol_concentration(self, signal: StrategySignal, portfolio: Optional[Portfolio]) -> bool:
