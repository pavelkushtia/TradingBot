...
class MeanReversionStrategy(BaseStrategy):
    ...
    Generates buy signals when price is oversold (low RSI) and near lower Bollinger Band.
    Generates sell signals when price is overbought (high RSI) and near upper Bollinger Band.
    """
    ...
    def __init__(self, name: str, parameters: Dict[str, Any]):
        ...
        self.min_price_distance = parameters.get("min_price_distance", 0.02)  # 2% from band
    ...
    async def _analyze_symbol(self, symbol: str) -> StrategySignal:
        ...
        if rsi <= self.rsi_oversold:
            ...
            distance_from_lower = (current_price - bollinger["lower"]) / bollinger["lower"]
            ...
            if distance_from_lower <= self.min_price_distance:
                ...
                band_strength = max(0, self.min_price_distance - distance_from_lower) / self.min_price_distance
        ...
        # Check for overbought conditions (potential sell signal)
            ...
            distance_from_upper = (bollinger["upper"] - current_price) / bollinger["upper"]
            ...
            if distance_from_upper <= self.min_price_distance:
                ...
                band_strength = max(0, self.min_price_distance - distance_from_upper) / self.min_price_distance
                strength = (rsi_strength + band_strength) / 2
