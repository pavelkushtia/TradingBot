...
from ..core.models import StrategySignal, MarketData, Quote
...
class StrategyManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            await self.add_strategy(
                self.config.strategy.default_strategy,
                self.config.strategy.parameters
            )
            
            self.logger.logger.info(f"Strategy manager initialized with {len(self.strategies)} strategies")
    ...
    async def add_strategy(self, strategy_name: str, parameters: Dict[str, Any]) -> None:
        """Add a new strategy."""
        if strategy_name not in self.strategy_classes:
            raise StrategyError(f"Unknown strategy: {strategy_name}")
        
        if strategy_name in self.strategies:
            self.logger.logger.warning(f"Strategy {strategy_name} already exists, replacing...")
    ...
    async def generate_signals(self) -> List[StrategySignal]:
        ...
        for strategy_name, strategy in self.strategies.items():
            try:
                ...
                self.logger.log_error(e, {"context": "signal_generation", "strategy": strategy_name})
    ...
    async def update_market_data(self, symbol: str, data: MarketData) -> None:
        ...
        for strategy in self.strategies.values():
            try:
                ...
                self.logger.log_error(e, {"context": "market_data_update", "symbol": symbol})
    ...
    async def update_quote(self, quote: Quote) -> None:
        ...
        for strategy in self.strategies.values():
            try:
                ...
                self.logger.log_error(e, {"context": "quote_update", "symbol": quote.symbol})
