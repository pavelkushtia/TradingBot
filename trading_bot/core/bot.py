...
from .exceptions import TradingBotError, RiskManagementError
from .logging import setup_logging, TradingLogger
from .models import Portfolio, Order, Position, StrategySignal
...
class TradingBot:
    ...
    async def _process_market_data(self) -> None:
        ...
        symbols = list(self.positions.keys()) + [order.symbol for order in self.active_orders.values()]
    ...
    async def _filter_signals(self, signals: List[StrategySignal]) -> List[StrategySignal]:
        ...
        for signal in signals:
            try:
                ...
                is_approved = await self.risk_manager.evaluate_signal(signal, self.portfolio)
                ...
                self.logger.log_error(e, {"context": "signal_filtering", "signal": signal.dict()})
    ...
    async def _execute_signals(self, signals: List[StrategySignal]) -> None:
        ...
        for signal in signals:
            try:
                ...
                self.logger.log_error(e, {"context": "signal_execution", "signal": signal.dict()})
    ...
    async def _check_risk_limits(self) -> None:
        ...
        daily_pnl_pct = (self.portfolio.total_value - self.start_portfolio_value) / self.start_portfolio_value
    ...
    async def _cancel_all_orders(self) -> None:
        ...
        for order_id, order in self.active_orders.items():
            try:
                ...
                self.logger.log_error(e, {"context": "cancel_order", "order_id": order_id})
    ...
    async def _close_all_positions(self) -> None:
        ...
        for symbol, position in self.positions.items():
            try:
                ...
                self.logger.log_error(e, {"context": "close_position", "symbol": symbol})
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "portfolio_value": str(self.portfolio.total_value) if self.portfolio else "0",
            "active_orders": len(self.active_orders),
